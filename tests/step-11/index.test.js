...
test('Parse SQL Query', () => {
    ...
    expect(parsed).toHaveProperty('table', 'student');
    expect(parsed).toHaveProperty('whereClauses', []);
    expect(parsed).toHaveProperty('joinType', null);
    expect(parsed).toHaveProperty('joinTable', null);
    expect(parsed).toHaveProperty('joinCondition', null);
});
...
test('Parse SQL Query with Multiple WHERE Clauses', () => {
    ...
    expect(parsed).toHaveProperty('fields', ['id', 'name']);
    expect(parsed).toHaveProperty('table', 'student');
    expect(parsed).toHaveProperty('whereClauses', [{ field: 'age', operator: '=', value: '30' }, { field: 'name', operator: '=', value: 'John' }]);
    expect(parsed).toHaveProperty('joinType', null);
    expect(parsed).toHaveProperty('joinTable', null);
    expect(parsed).toHaveProperty('joinCondition', null);
});
...
test('Parse SQL Query with INNER JOIN', async () => {
    ...
    expect(result).toHaveProperty('fields', ['student.name', 'enrollment.course']);
    expect(result).toHaveProperty('table', 'student');
    expect(result).toHaveProperty('whereClauses', []);
    expect(result).toHaveProperty('joinType', 'INNER');
    expect(result).toHaveProperty('joinTable', 'enrollment');
    expect(result).toHaveProperty('joinCondition', { left: 'student.id', right: 'enrollment.student_id' });
});
...
test('Parse SQL Query with INNER JOIN and WHERE Clause', async () => {
    ...
    expect(result).toHaveProperty('fields', ['student.name', 'enrollment.course']);
    expect(result).toHaveProperty('table', 'student');
    expect(result).toHaveProperty('whereClauses', [{ field: 'student.age', operator: '>', value: '20' }]);
    expect(result).toHaveProperty('joinType', 'INNER');
    expect(result).toHaveProperty('joinTable', 'enrollment');
    expect(result).toHaveProperty('joinCondition', { left: 'student.id', right: 'enrollment.student_id' });
});
...
test('Execute SQL Query with RIGHT JOIN with a WHERE clause filtering the main table', async () => {
    const query = 'SELECT student.name, enrollment.course FROM student RIGHT JOIN enrollment ON student.id=enrollment.student_id WHERE student.age < 25 AND student.name = Bob';
    const result = await executeSELECTQuery(query);
    expect(result).toEqual(expect.arrayContaining([
...
test('Parse SQL Query', () => {
    ...
    expect(parsed).toHaveProperty('joinTable', null);
    expect(parsed).toHaveProperty('joinCondition', null);
    expect(parsed).toHaveProperty('groupByFields', null);
});
...
test('Parse SQL Query with WHERE Clause', () => {
    ...
    expect(parsed).toHaveProperty('joinCondition', null);
    expect(parsed).toHaveProperty('groupByFields', null);
    expect(parsed).toHaveProperty('hasAggregateWithoutGroupBy', false);
});
...
test('Parse SQL Query with INNER JOIN', async () => {
    ...
    expect(result).toHaveProperty('joinTable', 'enrollment');
    expect(result).toHaveProperty('joinCondition', { left: 'student.id', right: 'enrollment.student_id' });
    expect(result).toHaveProperty('groupByFields', null);
    expect(result).toHaveProperty('hasAggregateWithoutGroupBy', false);
});
...
test("Parse INNER JOIN clause", () => {
  ...
  expect(result).toHaveProperty("joinType", "INNER");
  expect(result).toHaveProperty("joinTable", "table2");
  expect(result).toHaveProperty("joinCondition", {
    left: "table1.id",
    right: "table2.ref_id",
  });
});
...
test("Parse LEFT JOIN clause", () => {
  ...
  const result = parseJoinClause(query);
  expect(result).toHaveProperty("joinType", "LEFT");
  expect(result).toHaveProperty("joinTable", "table2");
  expect(result).toHaveProperty("joinCondition", {
    left: "table1.id",
    right: "table2.ref_id",
...
test("Parse RIGHT JOIN clause", () => {
  ...
  const result = parseJoinClause(query);
  expect(result).toHaveProperty("joinType", "RIGHT");
  expect(result).toHaveProperty("joinTable", "table2");
  expect(result).toHaveProperty("joinCondition", {
    left: "table1.id",
    right: "table2.ref_id",
...
test("Returns null for queries without JOIN", () => {
  ...
  const result = parseJoinClause(query);
  expect(result).toHaveProperty("joinType", null);
  expect(result).toHaveProperty("joinTable", null);
  expect(result).toHaveProperty("joinCondition", null);
});
...
test("Parse SQL Query with LEFT JOIN with a WHERE clause filtering the main table", async () => {
  ...
  expect(result).toHaveProperty("table", "student");
  expect(result).toHaveProperty("whereClauses", [
    { field: "student.age", operator: ">", value: "22" },
  ]);
  expect(result).toHaveProperty("joinType", "LEFT");
  expect(result).toHaveProperty("joinTable", "enrollment");
  expect(result).toHaveProperty("joinCondition", {
...
test("Parse SQL Query with RIGHT JOIN with a WHERE clause filtering the main table", async () => {
  ...
  expect(result).toHaveProperty("table", "student");
  expect(result).toHaveProperty("whereClauses", [
    { field: "student.age", operator: "<", value: "25" },
  ]);
  expect(result).toHaveProperty("joinType", "RIGHT");
  expect(result).toHaveProperty("joinTable", "enrollment");
  expect(result).toHaveProperty("joinCondition", {
...
test("Parse COUNT Aggregate Query", () => {
  const query = "SELECT COUNT(*) FROM student";
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["COUNT(*)"]);
  expect(parsed).toHaveProperty("table", "student");
  expect(parsed).toHaveProperty("whereClauses", []);
  expect(parsed).toHaveProperty("groupByFields", null);
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", true);
  expect(parsed).toHaveProperty("joinCondition", null);
  expect(parsed).toHaveProperty("joinTable", null);
  expect(parsed).toHaveProperty("joinType", null);
});
...
test("Parse SUM Aggregate Query", () => {
  const query = "SELECT SUM(age) FROM student";
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["SUM(age)"]);
  expect(parsed).toHaveProperty("table", "student");
  expect(parsed).toHaveProperty("whereClauses", []);
  expect(parsed).toHaveProperty("groupByFields", null);
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", true);
...
test("Parse AVG Aggregate Query", () => {
  const query = "SELECT AVG(age) FROM student";
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["AVG(age)"]);
  expect(parsed).toHaveProperty("table", "student");
  expect(parsed).toHaveProperty("whereClauses", []);
  expect(parsed).toHaveProperty("groupByFields", null);
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", true);
...
test("Parse MIN Aggregate Query", () => {
  const query = "SELECT MIN(age) FROM student";
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["MIN(age)"]);
  expect(parsed).toHaveProperty("table", "student");
  expect(parsed).toHaveProperty("whereClauses", []);
  expect(parsed).toHaveProperty("groupByFields", null);
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", true);
...
test("Parse MAX Aggregate Query", () => {
  const query = "SELECT MAX(age) FROM student";
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["MAX(age)"]);
  expect(parsed).toHaveProperty("table", "student");
  expect(parsed).toHaveProperty("whereClauses", []);
  expect(parsed).toHaveProperty("groupByFields", null);
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", true);
...
test("Parse basic GROUP BY query", () => {
  const query = "SELECT age, COUNT(*) FROM student GROUP BY age";
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["age", "COUNT(*)"]);
  expect(parsed).toHaveProperty("table", "student");
  expect(parsed).toHaveProperty("whereClauses", []);
  expect(parsed).toHaveProperty("groupByFields", ["age"]);
  expect(parsed).toHaveProperty("joinType", null);
  expect(parsed).toHaveProperty("joinTable", null);
  expect(parsed).toHaveProperty("joinCondition", null);
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", false);
...
test("Parse GROUP BY query with multiple fields", () => {
  ...
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["student_id", "course", "COUNT(*)"]);
  expect(parsed).toHaveProperty("table", "enrollment");
  expect(parsed).toHaveProperty("whereClauses", []);
  expect(parsed).toHaveProperty("groupByFields", ["student_id", "course"]);
  expect(parsed).toHaveProperty("joinType", null);
  expect(parsed).toHaveProperty("joinTable", null);
  expect(parsed).toHaveProperty("joinCondition", null);
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", false);
...
test("Parse GROUP BY query with JOIN and WHERE clauses", () => {
  ...
  const parsed = parseQuery(query);
  expect(parsed).toHaveProperty("fields", ["student.name", "COUNT(*)"]);
  expect(parsed).toHaveProperty("table", "student");
  expect(parsed).toHaveProperty("whereClauses", [
    { field: "enrollment.course", operator: "=", value: '"Mathematics"' },
  ]);
  expect(parsed).toHaveProperty("groupByFields", ["student.name"]);
  expect(parsed).toHaveProperty("joinType", "INNER");
  expect(parsed).toHaveProperty("joinTable", "enrollment");
  expect(parsed).toHaveProperty("joinCondition", {
    left: "student.id",
    right: "enrollment.student_id",
  });
  expect(parsed).toHaveProperty("hasAggregateWithoutGroupBy", false);
